...
>
> Updated for Everest Chronicle news-portal behavior: multi-category articles, primary category semantics, optional visible breadcrumbs, news-specific article headers, stricter date/image/author rules, and canonical consistency.
...
Every standalone story card should generally use `<article>`.
...
Do not globally hard-code every article card to H2 or H3.

The parent section determines the correct heading level.
...
If a reusable component receives its heading level from the parent, the semantic hierarchy stays correct across homepage, category, author, and related-story layouts.

Category labels shown above a card title are normally links or text labels, not headings.
...
# 15. Breadcrumb and Section Navigation Architecture

Breadcrumbs are useful for hierarchy, but a modern news article does **not** need a large visible breadcrumb trail if it harms editorial UX.

For Everest Chronicle:

- visible breadcrumb on article pages is optional;
- a compact visible category/section label is usually preferred;
- structured breadcrumb data may still communicate hierarchy;
- one article may belong to multiple categories;
- one primary category should be chosen for breadcrumb hierarchy and main editorial context.

## Recommended article UI

```text
ENVIRONMENT   EXPEDITION   CONSERVATION
...
The category labels are normal crawlable links.

```html
<div class="article-categories">
  <a href="/category/environment">Environment</a>
  <a href="/category/expedition">Expedition</a>
  <a href="/category/conservation">Conservation</a>
</div>
```

Do not use heading tags for these labels merely for styling.

## Primary category rule

If an article belongs to:
...
choose one category as primary:
...
Use the primary category for:

- breadcrumb hierarchy;
- primary editorial section context;
- optional lead category label;
- section-specific related-story logic where useful.
...
Do **not** generate a different canonical article URL per category.

## Visible breadcrumb when used

If the design uses a breadcrumb, keep it compact:
...
Do not force a visible breadcrumb if the page already has clear category context.

## Breadcrumb structured data

Structured breadcrumb data should normally reflect the primary category path.
  ...
  "itemListElement": [
    {
      ...
      "position": 1,
      "name": "Environment",
      "item": "https://everestchronicle.com/category/environment"
    },
    {
      "@type": "ListItem",
      "position": 2,
      "name": "Article Headline",
      "item": "https://everestchronicle.com/article/article-slug"
    }
  ]
}
```

Visible breadcrumb and `BreadcrumbList` are separate decisions.

A page may show only category links in the UI while still exposing a valid structured hierarchy.
...
All standard news detail pages should use a consistent editorial header contract.

## Required article-header order

```text
Primary/secondary category links
H1 headline
Standfirst / summary
Author / byline
Published date
...
│   ├── H2
│   └── article cards using H3 when nested beneath the H2
│
└── optional most-read / more-from-section modules
```
...
<main>
  <article>
    <header>
      ...
      <p class="standfirst">
        A short editorial summary of the story.
      </p>

      <p class="byline">
        By
        <a href="/author/author-name" rel="author">
          Author Name
        </a>
      </p>
...
## Article-header semantic rules

- category labels are links, not headings;
- byline is not a heading;
- publication date is not a heading;
- standfirst is normally a paragraph, not H2;
- article H1 is the primary headline;
- hero caption belongs in `<figcaption>`;
- body headings begin at H2 unless a real nested structure requires H3/H4.

---

# 19. NewsArticle Structured Data

For standard Everest Chronicle editorial news pages, use `NewsArticle` as the default article schema unless another supported article type more accurately represents the content.
...
Where practical, use the same preferred representative image across these outputs.
...
- modified date should change only for meaningful editorial updates;
- do not refresh dates merely to make content appear newer;
- keep visible dates and structured-data dates consistent;
- do not display an "Updated" date when the only change was technical formatting or metadata cleanup.

Recommended article display:

```text
Published September 25, 2026
Updated September 25, 2026, 2:35 PM
```

Only show the second line when the story has materially changed.
...
- the byline itself is not a heading;
- author names should link to a useful author profile when available;
- use `rel="author"` where appropriate;
- do not make the author name an H2 merely for styling;
- do not invent credentials or expertise.
...
# 38A. Related Stories and Editorial Modules

Use real section hierarchy for editorial modules.
...
- module title may be H2 when it is a major page section;
- nested article titles may be H3;
- category labels inside cards remain links/text, not headings;
- do not turn every small widget label into a heading;
- related-story links must be normal crawlable links.
...
SEO output should therefore derive contextual media fields such as:
...
from the relevant gallery usage/context record.
...
# 48A. Multi-Category Article Architecture

Everest Chronicle articles may belong to multiple categories.
...
```text
article_id    category_id    is_primary
120           1              1
120           2              0
120           3              0
```
...
- exactly one category should be designated primary when category context is important;
- all assigned categories may appear as crawlable article-header links;
- the primary category controls breadcrumb hierarchy and main section context;
- secondary categories must not create alternate canonical article URLs;
- article URLs remain category-independent;
- category membership must not change the article canonical.

Good:

```text
https://everestchronicle.com/article/climate-change-is-reshaping-everest-expeditions
```

Avoid:

```text
/category/environment/climate-change-is-reshaping-everest-expeditions
/category/expedition/climate-change-is-reshaping-everest-expeditions
/category/conservation/climate-change-is-reshaping-everest-expeditions
```

for the same canonical story.
...
{
  "id": 120,
  "title": "Climate Change Is Reshaping Everest Expeditions",
  "slug": "climate-change-is-reshaping-everest-expeditions",
  "primary_category": {
    "id": 1,
    "name": "Environment",
    "slug": "environment"
...
| Page type | Index? | H1 | Visible breadcrumb | Primary schema |
|---|---:|---:|---|---|
| Homepage | Yes | 1 | No | NewsMediaOrganization/WebSite as appropriate |
| Category | Yes | 1 | Optional | BreadcrumbList |
| Article | Yes | 1 | Optional/minimal | NewsArticle + BreadcrumbList |
| Author | Usually yes if useful | 1 | Optional | ProfilePage/Person when appropriate |
| Editorial topic | Only if useful | 1 | Optional | BreadcrumbList |
| Search results | Usually no | 1 | No | None required |
| Login | No | 1 | No | None |
| Account | No | 1 | No | None |
| Preview | No | 1 | No | None |
| 404 | No | 1 | No | None |

For article pages, a visible category/section label may replace a traditional breadcrumb in the UI while structured breadcrumb data still represents hierarchy.
...
- [ ] BreadcrumbList valid when used
- [ ] category listing links use canonical article URLs
...
- [ ] HTTP 200
- [ ] self-canonical
- [ ] one canonical URL regardless of category count
- [ ] indexable
- [ ] unique title
- [ ] unique description
- [ ] one article H1
- [ ] category labels are links, not headings
- [ ] one primary category selected when multiple categories exist
- [ ] visible author
...
- [ ] preferred hero/schema/social image signals are aligned where practical
- [ ] BreadcrumbList valid when used
...
same article resolves to multiple canonicals through category routes
article category label is rendered as heading only for styling
...
=
all category-page links pointing to that article
```
...
# 78A. News Portal Do-Not-Do Rules

Never:

- use H1 for the logo purely for styling;
- use H2/H3 for navigation items;
- use H2/H3 for category labels above article cards unless they truly introduce a section;
- hard-code every reusable article card to the same heading level;
- canonicalize category pages to the homepage;
- generate separate canonical article URLs for each category;
- expose localhost/staging URLs in production metadata;
- use `div onClick` for normal crawlable navigation;
- use `NewsArticle` schema on category listing pages;
- refresh publication dates to simulate freshness;
- update `dateModified` for trivial technical edits;
- index every tag/topic page automatically;
- index internal search result pages by default;
- canonicalize paginated page 2/3/4 to page 1 just because they share a category;
- include `noindex`, preview, search, 404, or noncanonical URLs in XML sitemaps;
- redirect unrelated deleted articles to the homepage;
- let client-side hydration replace correct server metadata with conflicting metadata;
- optimize toward arbitrary scanner counts instead of semantic correctness.
